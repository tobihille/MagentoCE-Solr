...
class DMC_Solr_Model_CatalogSearch_Mysql4_Advanced extends Mage_CatalogSearch_Model_Mysql4_Advanced
{
	...
	public function addIndexableAttributeFilter($object, $attribute, $value)
    {
		...
		if(Mage::helper('solr')->isEnabled()) {
			$typeConverter = new DMC_Solr_Document_TypeConverter($attribute->getFrontend()->getInputType());
    		if(isset($typeConverter->solr_search) && $typeConverter->solr_search) {

	    		$code = $attribute->getAttributeCode();
				$field = $typeConverter->solr_search_prefix.DMC_Solr_Document_TypeConverter::SUBPREFIX_SEARCH.$attribute->getAttributeCode();
