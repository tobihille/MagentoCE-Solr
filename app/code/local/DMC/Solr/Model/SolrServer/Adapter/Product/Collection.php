...
class DMC_Solr_Model_SolrServer_Adapter_Product_Collection extends Varien_Data_Collection
{
    /**
     * DB connection
     *
    ...
    protected function _getSearchableAttributes($backendType = null)
    {
        if (is_null($this->_searchableAttributes)) {
            ...
            $resource = Mage::getModel('Mage_CatalogSearch_Model_Mysql4_Fulltext_Collection');
            
            $whereCond  = array(
                $resource->getConnection()->quoteInto('additional_table.is_searchable=?', 1),
                $resource->getConnection()->quoteInto('main_table.attribute_code IN(?)', array('status', 'visibility'))
            );

            $select = $resource->getConnection()->select()
                ->from(array('main_table' => $resource->getTable('eav/attribute')))
                ->join(
                    array('additional_table' => $resource->getTable('catalog/eav_attribute')),
            ...
            foreach ($attributesData as $attributeData) {
                $attribute = Mage::getModel('catalog/entity_attribute')->loadByCode($entityType, $attributeData['attribute_code']);
